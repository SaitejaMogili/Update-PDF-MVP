...
import { FileText, Upload, Loader2, Download, CheckCircle, RotateCw } from "lucide-react";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/tools/rotate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fileId, angle, pages: pages.trim() || "all" }),
      });
      ...
      throw new Error("Timed out waiting for rotation");
    } catch (err) {
  ...
  if (phase === "done" && downloadUrl) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        ...
        <h2 className="mb-6 text-lg font-bold text-slate-900">Rotation complete!</h2>
        <div className="flex items-center justify-center gap-3">
          ...
          <Button variant="outline" onClick={reset}>Rotate another</Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        ...
        <p className="font-semibold text-slate-900">
          {phase === "uploading" ? "Uploading…" : "Rotating pages…"}
        </p>
      </div>
    );
  }
      ...
      {file && phase === "idle" && (
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="space-y-4 px-5 py-5">
            <div>
              <label className="mb-2 block text-xs font-semibold text-slate-700">Rotation angle</label>
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold text-slate-700">Pages</label>
              <input
                type="text"
                value={pages}
                onChange={(e) => setPages(e.target.value)}
                placeholder='e.g. "all", "1-3", "1,3,5-8"'
              ...
              />
              <p className="mt-1 text-[11px] text-slate-400">Use &ldquo;all&rdquo; for every page, or a range like 1-3,5,8.</p>
            </div>
          </div>
