...
import { FileText, Upload, Loader2, Download, CheckCircle, ScanText } from "lucide-react";
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      while (attempts < 60) {
        ...
        if (poll.status === "failed") throw new Error(poll.error ?? "OCR failed on server");
        attempts++;
      }
      throw new Error("Timed out waiting for OCR");
    } catch (err) {
  ...
  if (phase === "done" && downloadUrl) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-10 shadow-sm text-center">
        ...
        <h2 className="mb-1 text-lg font-bold text-slate-900">OCR complete!</h2>
        <p className="mb-6 text-sm text-slate-500">Your PDF is now searchable and copy-paste friendly.</p>
        <div className="flex items-center justify-center gap-3">
          ...
          <Button variant="outline" onClick={reset}>Run OCR on another</Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        ...
        <p className="font-semibold text-slate-900">
          {phase === "uploading" ? "Uploading…" : "Running OCR…"}
        </p>
        <p className="mt-1 text-sm text-slate-500">OCR on large documents can take up to a minute.</p>
      </div>
    );
  }
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          ...
          <div className="px-5 py-4 border-b border-slate-100">
            ...
            <select
              ...
              className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            >
              {LANGUAGES.map((l) => (
          ...
          <div className="flex items-center justify-between px-5 py-4">
            <p className="text-xs text-slate-400">Makes scanned PDFs searchable — 5 credits</p>
            <Button onClick={handleRun} className="gap-2 bg-blue-600 hover:bg-blue-700 text-white">
