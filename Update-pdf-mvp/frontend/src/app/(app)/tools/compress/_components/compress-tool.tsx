...
import { FileText, Upload, Loader2, Download, CheckCircle, Minimize2 } from "lucide-react";
...
function savings(before: number, after: number): string {
  ...
  return pct > 0 ? `${pct.toFixed(1)}% smaller` : "already optimised";
}
...
export function CompressTool() {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      const compressRes = await fetch("/api/tools/compress", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!compressRes.ok) {
        const { error } = await compressRes.json();
      ...
      while (attempts < 30) {
        ...
        if (poll.status === "done" && poll.downloadUrl) {
          ...
          setOutputSize(poll.outputSizeBytes ?? null);
          setPhase("done");
          return;
        }
        if (poll.status === "failed") {
          throw new Error(poll.error ?? "Compression failed on server");
        }
        attempts++;
      }

      throw new Error("Timed out waiting for compression");
    } catch (err) {
  ...
  // ── Done ─────────────────────────────────────────────────────
  if (phase === "done" && downloadUrl && file) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-10 shadow-sm">
        ...
        <h2 className="mb-1 text-center text-lg font-bold text-slate-900">Compression complete!</h2>

        {/* Before / after */}
        <div className="mx-auto mb-6 mt-4 flex max-w-xs items-center justify-center gap-4 rounded-xl border border-slate-100 bg-slate-50 px-6 py-4">
          <div className="text-center">
            <p className="text-xs text-slate-400">Before</p>
            <p className="mt-0.5 text-base font-semibold text-slate-700">{formatBytes(file.size)}</p>
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-4">
            <p className="text-xs text-slate-400">
              Ready to compress — 1 credit will be used
            </p>
            <Button
