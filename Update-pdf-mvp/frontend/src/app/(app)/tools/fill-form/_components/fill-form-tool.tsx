...
import { FileText, Upload, Loader2, Download, CheckCircle, PenLine } from "lucide-react";
...
export function FillFormTool() {
  ...
  const handleFill = async () => {
    ...
    if (!fields) {
      setParseError("Invalid format. Use one FieldName=Value per line.");
      return;
    }
    setParseError(null);
    setPhase("uploading");
    ...
    try {
      ...
      const res = await fetch("/api/tools/fill-form", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fileId, fields }),
      });
      if (!res.ok) {
        const { error } = await res.json();
        throw new Error(error ?? "Form fill failed");
      }
      ...
      while (attempts < 30) {
        ...
        if (poll.status === "failed") throw new Error(poll.error ?? "Form fill failed on server");
        attempts++;
      }
      throw new Error("Timed out waiting for form fill");
    } catch (err) {
  ...
  if (phase === "done" && downloadUrl) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-10 shadow-sm text-center">
        ...
        <h2 className="mb-1 text-lg font-bold text-slate-900">Form filled!</h2>
        <p className="mb-6 text-sm text-slate-500">Your PDF form has been filled and is ready to download.</p>
        <div className="flex items-center justify-center gap-3">
          ...
          <Button variant="outline" onClick={reset}>Fill another</Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        ...
        <p className="font-semibold text-slate-900">
          {phase === "uploading" ? "Uploading…" : "Filling form fields…"}
        </p>
    ...
    <div className="space-y-4">
      {!file && (
        ...
        >
          ...
          <p className="mb-1 text-base font-semibold text-slate-900">Drop your PDF form here</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          ...
          <div className="px-5 py-4 border-b border-slate-100">
            <label className="mb-1.5 block text-xs font-medium text-slate-600">
              Form fields <span className="text-slate-400">— one per line, format: FieldName=Value</span>
            </label>
            <textarea
              value={fieldsRaw}
              onChange={(e) => { setFieldsRaw(e.target.value); setParseError(null); }}
              rows={5}
              placeholder={"FirstName=John\nLastName=Doe\nEmail=john@example.com"}
              className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 font-mono text-sm text-slate-800 placeholder:text-slate-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
            />
            {parseError && <p className="mt-1 text-xs text-red-500">{parseError}</p>}
            <p className="mt-1.5 text-xs text-slate-400">
              Field names must match the PDF form field names exactly.
            </p>
          </div>

          <div className="flex items-center justify-between px-5 py-4">
            <p className="text-xs text-slate-400">Fills PDF form fields programmatically — 1 credit</p>
            <Button
              onClick={handleFill}
              disabled={!fieldsRaw.trim()}
