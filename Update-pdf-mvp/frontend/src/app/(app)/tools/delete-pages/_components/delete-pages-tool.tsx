...
import { FileText, Upload, Loader2, Download, CheckCircle, FileX } from "lucide-react";
...
export function DeletePagesTool() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setPhase("processing");
      const res = await fetch("/api/tools/delete-pages", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fileId, pages: pages.trim() }),
      });
  ...
  if (phase === "done" && downloadUrl) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        ...
        <h2 className="mb-6 text-lg font-bold text-slate-900">Pages removed!</h2>
  ...
  if (phase === "processing" || phase === "uploading") {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
        ...
        <p className="font-semibold text-slate-900">{phase === "uploading" ? "Uploading…" : "Removing pages…"}</p>
      </div>
    );
  }
      ...
      {file && phase === "idle" && (
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="px-5 py-5">
            <label className="mb-1 block text-xs font-semibold text-slate-700">Pages to remove</label>
            <input
            ...
            />
            <p className="mt-1 text-[11px] text-slate-400">Comma-separated. At least one page must remain.</p>
          </div>
          <div className="flex items-center justify-between border-t border-slate-100 px-5 py-4">
            <p className="text-xs text-slate-400">1 credit will be used</p>
            <Button onClick={handleSubmit} disabled={!pages.trim()} className="gap-2 bg-blue-600 hover:bg-blue-700 text-white">
              <FileX className="h-4 w-4" /> Remove pages
            </Button>
          </div>
        </div>
      )}
