...
import { watermarkPdf } from "@/lib/tools/watermark";

const CREDIT_COST = 1;
const TOOL = "watermark";

const bodySchema = z.object({
  fileId: z.string().uuid(),
  text: z.string().min(1).max(60),
  opacity: z.number().min(0.05).max(1).default(0.2),
...
export async function POST(request: Request) {
  ...
  const { fileId, text, opacity, color } = parsed.data;
  ...
  try {
    ...
    const outBytes = new Uint8Array(await watermarkPdf(inputBuffer, text, opacity, color));
    ...
    const { data: outFile } = await service.from("files").insert({
      user_id: user.id, filename: `${baseName}-watermarked.pdf`, size_bytes: outBytes.byteLength,
