...
import { runOcr } from "@/lib/tools/ocr";
...
export async function POST(request: Request) {
  ...
  const { fileId, language } = parsed.data;
  ...
  const debit = await debitCredits(user.id, CREDIT_COST, "ocr", undefined);
  ...
  const { data: job, error: jobError } = await service.from("tool_jobs").insert({
    user_id: user.id, tool_slug: "ocr", input_file_ids: [fileId],
    status: "processing", credits_charged: CREDIT_COST, started_at: new Date().toISOString(),
  }).select("id").single();

  if (jobError || !job) {
    await refundCredits(user.id, CREDIT_COST, "ocr_job_create_failed");
  ...
  try {
    ...
    const resultUrl = await runOcr(signed.signedUrl, language);
    ...
    const { data: outputFile } = await service.from("files").insert({
      user_id: user.id, filename: `${baseName}-searchable.pdf`, size_bytes: outputBytes.byteLength,
    ...
    await refundCredits(user.id, CREDIT_COST, "ocr_failed", job.id);
