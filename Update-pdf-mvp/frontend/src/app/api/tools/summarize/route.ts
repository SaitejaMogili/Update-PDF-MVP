...
import { summarizePdf } from "@/lib/tools/summarize";
...
export async function POST(request: Request) {
  ...
  if ((file.size_bytes ?? 0) > MAX_FILE_SIZE) {
    return NextResponse.json({ error: "File too large (max 10 MB for AI summarization)" }, { status: 413 });
  }

  const debit = await debitCredits(user.id, CREDIT_COST, "summarize", fileId);
  if (!debit.ok) {
  ...
  if (jobError || !job) {
    await refundCredits(user.id, CREDIT_COST, "summarize_job_create_failed");
  ...
  try {
    ...
    const result = await summarizePdf(pdfBuffer);
    ...
    await refundCredits(user.id, CREDIT_COST, "summarize_failed", job.id);
    await service
      .from("tool_jobs")
