...
export async function POST(request: Request) {
  ...
  const { fileId, password } = parsed.data;
  ...
  const debit = await debitCredits(user.id, CREDIT_COST, "protect", undefined);
  ...
  const { data: job, error: jobError } = await service.from("tool_jobs").insert({
    user_id: user.id, tool_slug: "protect", input_file_ids: [fileId],
    status: "processing", credits_charged: CREDIT_COST, started_at: new Date().toISOString(),
  }).select("id").single();

  if (jobError || !job) {
    await refundCredits(user.id, CREDIT_COST, "protect_job_create_failed");
  ...
  try {
    ...
    const resultUrl = await protectPdf(signed.signedUrl, password);
    ...
    const { data: outputFile } = await service.from("files").insert({
      user_id: user.id, filename: `${baseName}-protected.pdf`, size_bytes: outputBytes.byteLength,
    ...
    await refundCredits(user.id, CREDIT_COST, "protect_failed", job.id);
    ...
    return NextResponse.json({ error: "Protection failed" }, { status: 500 });
  }
}
