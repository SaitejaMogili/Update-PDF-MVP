...
const bodySchema = z.object({
  ...
  pages: z.string().min(1).max(200).default("all"),
});
...
export async function POST(request: Request) {
  ...
  const { fileId, angle, pages } = parsed.data;
  ...
  try {
    ...
    const outBytes = new Uint8Array(await rotatePdf(inputBuffer, angle, pages));
    ...
    const { data: outFile } = await service.from("files").insert({
      user_id: user.id, filename: `${baseName}-rotated.pdf`, size_bytes: outBytes.byteLength,
