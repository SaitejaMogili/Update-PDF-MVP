...
import { addPageNumbers } from "@/lib/tools/page-numbers";

const CREDIT_COST = 1;
const TOOL = "page-numbers";

const bodySchema = z.object({
  fileId: z.string().uuid(),
  position: z.enum(["header", "footer"]).default("footer"),
  alignment: z.enum(["left", "center", "right"]).default("center"),
  startAt: z.number().int().min(1).max(9999).default(1),
  fontSize: z.number().int().min(6).max(48).default(10),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const outBytes = new Uint8Array(await addPageNumbers(inputBuffer, opts));
    ...
    const { data: outFile } = await service.from("files").insert({
      user_id: user.id, filename: `${baseName}-numbered.pdf`, size_bytes: outBytes.byteLength,
    ...
    return NextResponse.json({ error: "Page numbering failed" }, { status: 500 });
  }
}
