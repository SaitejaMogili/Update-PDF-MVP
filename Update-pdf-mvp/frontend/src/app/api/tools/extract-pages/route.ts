...
import { extractPages } from "@/lib/tools/extract-pages";

const CREDIT_COST = 1;
const TOOL = "extract-pages";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const outBytes = new Uint8Array(await extractPages(inputBuffer, pages));
    ...
    const { data: outFile } = await service.from("files").insert({
      user_id: user.id, filename: `${baseName}-extracted.pdf`, size_bytes: outBytes.byteLength,
    ...
    return NextResponse.json({ error: "Extract pages failed" }, { status: 500 });
  }
}
