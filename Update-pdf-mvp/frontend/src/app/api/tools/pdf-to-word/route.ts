...
import { convertToWord } from "@/lib/tools/pdf-to-word";
...
export async function POST(request: Request) {
  ...
  const debit = await debitCredits(user.id, CREDIT_COST, "pdf-to-word", undefined);
  ...
  const { data: job, error: jobError } = await service.from("tool_jobs").insert({
    user_id: user.id, tool_slug: "pdf-to-word", input_file_ids: [fileId],
    status: "processing", credits_charged: CREDIT_COST, started_at: new Date().toISOString(),
  }).select("id").single();

  if (jobError || !job) {
    await refundCredits(user.id, CREDIT_COST, "pdf-to-word_job_create_failed");
  ...
  try {
    ...
    const resultUrl = await convertToWord(signed.signedUrl);
    ...
    const outputPath = `${user.id}/${job.id}.docx`;
    const { error: uploadError } = await service.storage.from("outputs")
      .upload(outputPath, outputBytes, { contentType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document", upsert: false });
    ...
    const { data: outputFile } = await service.from("files").insert({
      user_id: user.id, filename: `${baseName}.docx`, size_bytes: outputBytes.byteLength,
      mime_type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
      storage_path: outputPath, expires_at: new Date(Date.now() + 60 * 60 * 1000).toISOString(),
    }).select("id").single();
    ...
    await refundCredits(user.id, CREDIT_COST, "pdf-to-word_failed", job.id);
